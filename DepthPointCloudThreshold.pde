...
//Min and max thresholds for model scanning
float minThreshold = 200;
float maxThreshold = 1900;

// File to write point cloud data to
PrintWriter output;

//Number of Points
int trueCount = 217088;
int ourCount = 54272;
...
void draw() {
  ...
  output = createWriter("positions4.txt"); // change this every time to avoid overwriting old file saved 
    
   
    output.println("# .PCD v.7 - Point Cloud Data file format\n" +
          "VERSION .7\n" +
          "FIELDS x y z\n" +
  ...
  if (renderMode == 1) {
    ...
    for (int x = 0; x < kinect2.depthWidth; x+=skip) {
      for (int y = 0; y < kinect2.depthHeight; y+=skip) {
        ...
        output.println(" " + point.x + " " + point.y + " " + point.z); // Write the coordinate to the file
          }
        //end of new/modified stuff
